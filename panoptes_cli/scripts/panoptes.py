...
def get_collaborator(role_id):
    """Takes a role id, and returns an object containing the user information
...
def project(id, display_name, slug, sets, roles, verbose):
    ...
    for proj_data in projects['projects']:
        ...
        click.echo('\tClassification count: %s' %
                   proj_data['classifications_count'])
        ...
        if sets or verbose:
            ...
            subject_sets = map(lambda id: panoptes.get_subject_set(id)
                               ['subject_sets'][0],
        ...
        if roles or verbose:
            ...
            collaborators = pool.map(get_collaborator, proj_data['links']
                                     ['project_roles'])
